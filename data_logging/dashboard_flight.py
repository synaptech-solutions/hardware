"""Interactive flight-data dashboard for combine_flight.py output (flight_synced.mat).

A Plotly Dash web app — opens in your browser, nothing is written to disk (use
plot_flight.py if you want static PNGs). Features:
  - linked-axis zoom/pan across all time-series + a range slider on the bottom
    plot to focus on a section of the flight,
  - a checklist to choose which panels are shown,
  - a smooth WebGL 3D trajectory you can rotate / pan / zoom (drag, shift-drag,
    scroll), colorable by time / mean RPM / speed.

Reuses plot_flight.py's loader + discovery so it reads the exact same files.
...
Usage (run with the repo venv):
  ...
  .venv/bin/python data_logging/dashboard_flight.py recordings/X/flight_synced.mat
...
from plot_flight import load_synced, list_synced, select_synced, _euler_deg, _mean_rpm
...
DEFAULT_ON = ["pos", "vel", "orient", "rpm", "altrpm"]
...
def build_timeseries(d, selected):
    ...
    # Range slider on the bottom axis for focusing on a section; shared_xaxes
    # makes zoom/pan on any panel apply to all.
    ...
    fig.update_layout(
        ...
        uirevision="keep",          # preserve zoom across panel toggles
    )
    return fig
...
def build_3d(d, color_by):
    ...
    fig.update_layout(
        ...
        scene=dict(xaxis_title="x (m)", yaxis_title="y (m)", zaxis_title="z (m)",
                   aspectmode="data"),
        uirevision="keep3d",        # preserve camera across recolors
        title="3D trajectory  (drag = rotate · shift-drag = pan · scroll = zoom)")
    return fig


def header_text(d, path):
    bits = [f"file: {os.path.relpath(path)}"]
    for k, lbl in (("session", "session"), ("exptime", "t0"),
                   ("sync_method", "sync"), ("blackbox_file", "bbl"),
                   ("video_file", "video")):
        v = d.get(k)
        if v:
            bits.append(f"{lbl}: {v}")
...
def main():
    ...
    ap.add_argument("file", nargs="?", help="flight_synced.mat (default: newest)")
    ...
    app = make_app(d, path, "Flight dashboard")
