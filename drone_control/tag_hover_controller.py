...
Vertical uses an ASYMMETRIC throttle trim (small upward, larger downward)
so a tag falling out of view can't command a runaway climb. No integral —
it wound up to its cap before reaching target and made every climb overshoot;
tune HOVER_THROTTLE_US for steady-state hover.
...
# Hover goal: rise to the tag altitude, then hold the same XY position where
# the drone armed (no recentering behavior).
TARGET_LAT_M = 0.0
TARGET_UP_M = 0.0          # 0 = level with tag (tag height)
TARGET_FWD_FALLBACK_M = 0.50   # hover distance from tag (used unless arm-time lock)
LOCK_LAT_AT_ARM = True     # hold arm-time lateral offset
LOCK_FWD_AT_ARM = False    # False → always hover at TARGET_FWD_FALLBACK_M
YAW_NEUTRAL_HOLD = False   # True = neutral yaw (stability-first; avoid yaw-induced tag loss)
...
# Vision input. Must match the resolution used by camera_setup/calibrate_camera.py
# — K is in PIXELS at the calibrated resolution, and using a different capture
# size makes solvePnP return distances off by the resolution ratio.
DEVICE_INDEX = 4
...
# Stick signs — match setup/test_stick_directions.py (which sends 1700 us
# for "PITCH FORWARD" / "ROLL RIGHT" / "YAW RIGHT", confirmed in Betaflight
# Configurator). Standard AETR convention: RC pitch >1500 = nose-down = fwd.
# The 2026-05-29 flight log showed fwd growing while the controller commanded
# forward (RC pitch 1482) — the previous SIGN_PITCH=-1 inverted the axis.
SIGN_PITCH = -1
SIGN_ROLL = +1
SIGN_YAW = +1
...
# Hover throttle and limits. There is no integrator (see PoseFilter docstring),
# so a wrong value shows up as steady-state climb (too high) or sag (too low).
# Refine after every flight: in the 2026-05-28 simple-hover flight the drone
# stabilised at T≈1338-1342 with v_up≈0 on a 4.10V battery → 1340 is the
# observed baseline.
...
KV_LAT_US_PER_MPS = 52.0    # roll us per (m/s) of v-error
...
# Deadband suppresses chasing tiny lateral offsets (PnP corner jitter +
# imperfect tag placement read as "off-center" by a few degrees even when
# the tag looks centered to the eye).
KP_YAW_US_PER_RAD = 120.0
MAX_YAW_US = 45
YAW_BEARING_DEADBAND_RAD = math.radians(2.5)
...
DISPLAY_HZ = 30.0
LOG_HZ = 12.0
LOG_EVERY_LOOP = False  # Per-loop logging can starve vision; use throttled logs by default.
...
def vision_loop(shared, tx, K, dist, cam_params):
    ...
    def select_target(detections):
        cands = [d for d in detections if d.tag_id == TARGET_TAG_ID]
        if not cands:
            return None, 0
        return max(cands, key=lambda d: float(getattr(d, "decision_margin", 0.0))), len(cands)
    ...
    while shared.running:
        ...
        # Pass 2: contrast boost fallback (every few misses).
        ...
        # Pass 3: local ROI around last good center.
        ...
        if selected is not None:
            ...
            if not plausible:
                reasons = []
                if dm < MIN_TAG_DECISION_MARGIN:
                    reasons.append("low_dm")
                if not (0.10 <= fwd <= MAX_TRACK_RANGE_M):
                    reasons.append("fwd_range")
                if abs(lat) > MAX_TRACK_LAT_M:
                    reasons.append("lat_range")
                if abs(up) > MAX_TRACK_UP_M:
                    reasons.append("up_range")
                reject_reason = "+".join(reasons) if reasons else "rejected"

            vis_dbg = (
                f"pass={detect_label or 'none'} cands={cand_count} "
            ...
            if plausible:
                ...
                cv2.putText(frame,
                            ...
                            f"dm={dm:.0f} [{detect_label} id={int(selected.tag_id)}]",
            ...
            with shared.lock:
                shared.has_tag = False
                shared.vision_dbg = (
                    f"pass=none cands=0 sel_id=-1 dm=0.0 plaus=0 reason=no_target "
                    f"miss={miss}"
                )
...
def main():
    ...
    def axis_lean_us(pos, vel, target, kp, kv, vmax):
        ...
        e_pos = pos - target
        v_des = clamp(-kp * e_pos, -vmax, vmax)
        return kv * (vel - v_des)   # vel here is TAG-frame d(pos)/dt

    def vertical_throttle(up, v_up, dt, target_up):
        """Velocity-loop PD with ASYMMETRIC clamp (smaller upward authority
        than downward) — vision-only altitude must not run away upward, but
        can react to a fall while the tag stays in frame. No integral term:
        it wound up to its cap before the drone reached the target and made
        every climb overshoot. Tune HOVER_THROTTLE_US for steady-state hover."""
        e_pos = up - target_up
        v_des = clamp(-KP_UP * e_pos, -VMAX_UP_MPS, VMAX_UP_MPS)
        e_v = v_up - v_des             # > 0 means we're below target / not climbing fast enough
        thr_corr = KV_UP_US_PER_MPS * e_v
        thr_corr = clamp(thr_corr, -THR_DESC_TRIM_US, THR_CLIMB_TRIM_US)
        return round(clamp(HOVER_THROTTLE_US + thr_corr,
    ...
    def yaw_us(fwd, lat):
        ...
        if abs(bearing) < YAW_BEARING_DEADBAND_RAD:
            return NEUTRAL_US
        u = clamp(KP_YAW_US_PER_RAD * bearing, -MAX_YAW_US, MAX_YAW_US)
        return round(NEUTRAL_US + SIGN_YAW * u)

    def slew_thr(desired, prev):
        step = THR_SLEW_US_PER_S * tx_period
        return round(clamp(desired, prev - step, prev + step))
    ...
    def throttle_cmd_label(us, deadband_us=5):
        # Compare to hover baseline so the command intent is obvious in flight.
        d = int(us - HOVER_THROTTLE_US)
        mag = abs(d)
        if mag <= deadband_us:
    ...
    try:
        while shared.running:
            ...
            if state in ("DRYRUN", "ARM_SETTLE"):
                # Always reset pose filter on fresh vision in DRYRUN, and update every loop
                if vis_fresh:
            ...
            e_v_up = 0.0
            thr_raw = float(thr)
            thr_after_slew = float(thr)
            lost = 0.0

            if state == "DRYRUN":
                if pose_filter.inited:
                    # Use the same targets the FLY state uses so the DRY printout
                    # is representative of the actual control output in flight.
                    ...
                    roll_err = est_lat - target_lat_m
                    pitch_err = est_fwd - target_fwd_m
                    roll = round(NEUTRAL_US + SIGN_ROLL * u_roll)
                    pitch = round(NEUTRAL_US + SIGN_PITCH * u_pitch)
                    yaw = NEUTRAL_US if YAW_NEUTRAL_HOLD else yaw_us(est_fwd, est_lat)
                    e_up = est_up - target_up_m
                    v_des_up = clamp(-KP_UP * e_up, -VMAX_UP_MPS, VMAX_UP_MPS)
                    e_v_up = v_up - v_des_up
                    thr_raw = vertical_throttle(est_up, v_up, dt, target_up_m)
                    thr = thr_raw
                    bearing_deg = math.degrees(math.atan2(est_lat, est_fwd)) if est_fwd > 0.10 else 0.0
                    note = (f"DRY est=({est_fwd:+.2f},{est_lat:+.2f},{est_up:+.2f}) "
                            f"v=({v_fwd:+.2f},{v_lat:+.2f},{v_up:+.2f}) "
                            f"tgt_fwd={target_fwd_m:+.2f} bearing={bearing_deg:+.1f}deg")
                else:
                    ...
                    if is_armed(flight_mode):
                        ...
                        if strong:
                            ...
                            if lock_s < ARM_LOCK_MIN_S:
                                ...
                                target_fwd_m = (float(vis_pos[0]) if LOCK_FWD_AT_ARM
                                                else TARGET_FWD_FALLBACK_M)
                                if LOCK_LAT_AT_ARM:
                                    target_lat_m = float(vis_pos[1])
                                else:
                                    target_lat_m = TARGET_LAT_M
                                ...
                                log(f">> FLY: target fwd={target_fwd_m:+.2f} lat={target_lat_m:+.2f} up=0  "
                ...
                if est_age < STATE_MAX_AGE_S:
                    ...
                    e_v_up = v_up - v_des_up
                    thr_raw = vertical_throttle(est_up, v_up, dt, target_up_m)
                    thr = slew_thr(thr_raw, last_thr)
                    thr_after_slew = float(thr)
                    last_thr = thr
                    bearing_deg = math.degrees(math.atan2(est_lat, est_fwd)) if est_fwd > 0.10 else 0.0
                    note = (f"fly fwd={est_fwd:+.2f}(t{target_fwd_m:+.2f}) "
                            f"lat={est_lat:+.2f}(t{target_lat_m:+.2f}) up={est_up:+.2f}  "
                            f"v=({v_fwd:+.2f},{v_lat:+.2f},{v_up:+.2f}) "
                    ...
                    t = round(clamp(HOVER_THROTTLE_US - bleed,
            ...
            if frame is not None:
                ...
                # Human-readable live command panel: direction + magnitude.
                panel_lines = [
                    axis_cmd_label(ch[CH_ROLL], "ROLL RIGHT", "ROLL LEFT"),
                    axis_cmd_label(ch[CH_PITCH], "PITCH UP", "PITCH DOWN"),
                    axis_cmd_label(ch[CH_YAW], "YAW RIGHT", "YAW LEFT"),
                    throttle_cmd_label(ch[CH_THR]),
                ]
                y0 = 58
            ...
            log_due = LOG_EVERY_LOOP or (now - last_log_t >= (1.0 / max(LOG_HZ, 1.0)))
            if note and log_due:
                ...
                est_txt = (f"est_age={est_age*1000:.0f}ms dt={dt*1000:.1f}ms "
                           f"vdes_up={v_des_up:+.3f} ev_up={e_v_up:+.3f} e_up={e_up:+.3f} "
                           f"roll_err={roll_err:+.3f} pitch_err={pitch_err:+.3f} "
                           f"u_roll={u_roll:+.1f} u_pitch={u_pitch:+.1f} "
                           f"thr_raw={thr_raw:.1f} thr_slew={thr_after_slew:.1f} "
                           f"lost={lost:.3f}")
                log(f"[{state}] {note}  -> R{ch[CH_ROLL]} P{ch[CH_PITCH]} "
                    f"Y{ch[CH_YAW]} T{ch[CH_THR]}  {vis_txt}  {imu_txt}  {est_txt}  "
                    f"vdbg=[{vis_dbg}] vis=[{shared.vision_status}] mode={mode_txt} batt={batt_txt}")
                last_log_t = now
